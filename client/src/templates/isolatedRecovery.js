...
const GAP = 60;
const NET_ROW = 200;              // row height in the middle fabric column
const MID_W   = 240;              // width reserved for the middle column

const Z_PROD = { x: 0, y: 100 };
const MID_X  = Z.prod.width + GAP;
const Z_IRE  = { x: MID_X + MID_W + GAP, y: 100 };

const MGMT_Y = Math.max(Z_PROD.y + Z.prod.height, Z_IRE.y + Z.ire.height) + GAP;
...
export default {
  ...
  graph: {
    nodes: [
      ...
      // Middle fabric column (between the two zones) — one device per row,
      // no overlap.
      netNode('prod-fw',   'firewall',        MID_X, Z_PROD.y + 0 * NET_ROW, { label: 'Prod Firewall',        risk: 'medium' }),
      netNode('air-in',    'boundary-input',  MID_X, Z_PROD.y + 1 * NET_ROW, { label: 'Replication Ingress',   capacity: 'Scheduled only', phase: 'Current' }),
      netNode('air-relay', 'cloud',           MID_X, Z_PROD.y + 2 * NET_ROW, { label: 'One-way Diode',          capacity: 'Scheduled' }),
      netNode('ire-fw',    'firewall',        MID_X, Z_PROD.y + 3 * NET_ROW, { label: 'Vault Firewall',         capacity: 'Ingress-only', risk: 'low', phase: 'Proposed' }),

      // OOB management below the zones
      netNode('mgmt-jump', 'server', Z_IRE.x,       MGMT_Y, { label: 'Jump Host (OOB)', capacity: 'MFA · PAM', phase: 'Proposed' }),
      netNode('mgmt-siem', 'server', Z_IRE.x + 240, MGMT_Y, { label: 'SIEM Forwarder',  capacity: 'Read-only logs', phase: 'Proposed' }),
    ],
    edges: [
      // Production zone → Prod FW → diode → IRE FW → IRE zone
